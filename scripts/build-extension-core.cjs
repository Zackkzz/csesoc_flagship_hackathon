#!/usr/bin/env node
/**
 * Generate extension/lib/llmguide-core.js from src/shared/core.ts.
 *
 * The extension has no bundler, so the shared core is transpiled to a plain
 * script that exposes a single `LLMGuideCore` global. The generated file is
...
const outPath = path.join(root, 'extension', 'lib', 'llmguide-core.js');
const tscBin = path.join(root, 'node_modules', '.bin', process.platform === 'win32' ? 'tsc.cmd' : 'tsc');

function generate() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'llmguide-core-'));
  try {
    ...
    return [
      ...
      'var LLMGuideCore = (function () {',
      '  var exports = {};',
      '  var module = { exports: exports };',
      transpiled.replace(/^/gm, '  ').replace(/[ \t]+$/gm, ''),
      '  return module.exports;',
      '})();',
      "if (typeof globalThis !== 'undefined') { globalThis.LLMGuideCore = LLMGuideCore; }",
      '',
    ].join('\n');
  } finally {
...
if (process.argv.includes('--check')) {
  ...
  if (existing !== output) {
    console.error('extension/lib/llmguide-core.js is out of sync with src/shared/core.ts.');
